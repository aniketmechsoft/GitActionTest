...
/**
 * BaseTest class serves as the foundation for all Appium test classes.
 * It handles platform detection, driver setup, and lifecycle management
...
public class BaseTest extends AppiumUtils {
     ...
     * @throws Exception if setup fails or configuration is missing.
     */
    @Parameters({ "platform" })
    ...
    public void configureAppium(@Optional String platformParam) throws Exception {
        ...
        String platformType = System.getProperty("platform", platformParam);
        ...
        try (FileInputStream fis = new FileInputStream("src/main/resources/configfile/appiumConfig.properties")) {
            prop.load(fis);
        }
    ...
    /**
     * Determines which platform to run tests on, based on system property,
     * TestNG parameter, or default OS.
     *
    ...
    /**
     * Performs iOS-specific driver setup using XCUITest.
     * Loads iOS configuration values from a properties file
     * and initializes the IOSDriver instance.
     *
    ...
    /**
     * Performs Android-specific driver setup using UiAutomator2.
     * Loads Android configuration values from a properties file
     * and initializes the Android driver session.
     *
     * @throws Exception if configuration or driver initialization fails.
     */
    private void setupAndroid() throws Exception {
        Properties prop = new Properties();
        try (FileInputStream fis = new FileInputStream("src/main/resources/configfile/androidWindowsConfig.properties")) {
            prop.load(fis);
        }
        ...
        driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(
                Long.parseLong(prop.getProperty("androidImplicitWait"))));

        System.out.println("Android session started on " + prop.getProperty("androidDeviceName"));
    }

    /**
     * Cleans up test resources after all tests are executed.
     * Ensures that both driver and Appium service are stopped properly.
     */
//    @AfterClass(alwaysRun = true)
    public void tearDown() {
        try { if (driver != null) driver.quit(); } catch (Exception ignored) {}
        try { if (service != null && service.isRunning()) service.stop(); } catch (Exception ignored) {}
    }
}
